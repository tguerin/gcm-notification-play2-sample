...
object NotificationSender {
  ...
  val Sender: Sender = new Sender("YOUR_API_KEY")

  def push(notification: Notification, regIdsList: Array[String]): List[Promise[MulticastResult]] = {
    val message = notification.asMessage
    (regIdsList chunk MaxMulticastSize map (regIds => {Akka future (Sender send(message, regIds.toList, 5))})).toList
  }
